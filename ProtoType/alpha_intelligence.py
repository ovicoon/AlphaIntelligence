...
class Connector:
    ...
    def learn(self, signal, state_layers_dict):
        # Store only active states (compressed)
        state_snapshot = {
            char: layer.signal
...
class AlphaIntelligence:
    ...
    def generate(self, question, max_length=4096, verbose=False):
        ...
        for _ in range(max_length):
            ...
            if strongest_char == last_char:
                ...
                if repeat_count >= 3:
                    if verbose:
                        print("  [repeat detected]", flush=True)
                    break
            else:
                repeat_count = 0
    ...
    def save(self, filepath, compression="bz2"):
        save_data = {
            "version": VERSION,
            "config": {
...
def learn_from_csv(model, file_path, max_dialogs=None):
    ...
    if os.path.isdir(file_path):
        print(f"  Error: path is a directory: {file_path}")
        return
    ...
    print(f"  Learning from {os.path.basename(file_path)} ({limit_text} dialogs)...")
    ...
    with open(file_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if max_dialogs:
                suffix = f"{total_learned}/{max_dialogs} dialogs, {total_pairs} pairs"
                print_progress_bar(total_learned, max_dialogs, suffix=suffix)
    ...
    print(
        f"  Done: {total_learned} dialogs, {total_pairs} pairs, {stats['chars']} chars ({elapsed:.1f}s)"
    )
...
HELP_TEXT = """
  Commands:
    learn "Q" "A"               learn a single Q/A pair
    learncsv <file> [count]     learn from CSV file
    stimulate "Q"               generate a response
    save <file> [compression]   save model  (gzip|bz2|lzma|none)
    load <file>                 load model
    reset                       reset all signals
        ...
        try:
            ...
            # ----------------------------------------------------------------
                ...
                if compression not in ("gzip", "bz2", "lzma", "none"):
                    print("  Error: compression must be one of: gzip, bz2, lzma, none")
                    continue
            ...
            # ----------------------------------------------------------------
                ...
                print(f"  Avg history     {stats['avg_history']:.1f}")
                print()
