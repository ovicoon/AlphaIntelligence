...
# Core parameters (will be randomized in auto mode)
DECAY_RATE = 0.781
...
class Connector:
    ...
    def __init__(self, output_layer_id):
        ...
        self.history_signals = []  # Sorted list
        self.history_states = []
...
def reset_connectors():
    for layer in input_layers_list:
        for conn in layer.connections:
            conn.history_signals = []
            conn.history_states = []


def set_random_hyperparameters():
    global DECAY_RATE, STATE_DECAY_RATE, SIMILARITY_K, STATE_INFLUENCE
    DECAY_RATE = random.uniform(0.5, 0.99)
    STATE_DECAY_RATE = random.uniform(0.01, 0.5)
    SIMILARITY_K = random.uniform(1.0, 100.0)
    STATE_INFLUENCE = random.uniform(0.0, 1.0)
...
def learn(question, answer):
    ...
    missing_chars = [char for char in question if char not in input_layers]
    if missing_chars:
        print(f"❌ Question 미등록 문자: {list(set(missing_chars))}")
        return
...
def parse_dialog(raw_text):
    ...
    return [text.strip() for text in dialog if text.strip()]


def learn_from_csv(file_path, max_dialogs=None, print_progress=True):
...
def auto_mode():
    print("🤖 Auto mode started (Ctrl+C to stop)")
    try:
        while True:
            print("\n--- New Iteration ---")
            learn_from_csv(
                "Alpha/DataSet/archive/train.csv", max_dialogs=10, print_progress=False
            )
            question = "Say, Jim, how about a few beer after dinner?"
            for char in question:
                stimulate(char)
                step_all_layers()
            generate_response()
            print("\n⚙️ Current Hyperparameters:")
            print(f"  - DECAY_RATE: {DECAY_RATE:.3f}")
            print(f"  - STATE_DECAY_RATE: {STATE_DECAY_RATE:.3f}")
            print(f"  - SIMILARITY_K: {SIMILARITY_K:.3f}")
            print(f"  - STATE_INFLUENCE: {STATE_INFLUENCE:.3f}")
            reset_connectors()
            set_random_hyperparameters()
            print("\n🔄 Hyperparameters randomized for next iteration")
...
def main():
    print("=" * 60)
    print("AlphaIntelligence v3.0 - Auto Mode Optimized")
    print("🚀 O(N) → O(log N) 속도 개선 + Auto Mode")
    print("=" * 60)
    print("Commands:")
    ...
    while running:
        try:
            ...
            if cmd == "exit":
                ...
                matches = re.findall(r'"([^"]*)"', rest)
                if len(matches) >= 2:
                    question, answer = matches[0], matches[1]
